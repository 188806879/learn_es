...
def reindex(settings, movieDict={}):
    resp = requests.delete("http://node01:9200/" + indexName)  # D
    data = json.dumps(settings, indent=4, sort_keys=True)
    print "settings:\n%s" % data
    resp = requests.put("http://node01:9200/" + indexName,
    ...
    resp = requests.post("http://node01:9200/_bulk", headers={"content-type": "application/json"}, data=bulkMovies)
    #  print resp.content
